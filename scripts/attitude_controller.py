...
class Edrone():
    ...
    def __init__(self):
        ...
        self.Ki = [0, 0, 0]
        self.Kd = [0, 0, 0]
        # -----------------------Added other required variables for pid here ----------------------------------------------
        #
    ...
    def imu_callback(self, msg):
        ...
        rospy.loginfo("IMU Callback Message is:" + str(msg))
    ...
    # ----------------------------Defined callback function like roll_set_pid to tune pitch, yaw--------------
    ...
    # Callback function for /pid_tuning_throttle
    def throttle_set_pid(self, throttle):
        self.Kp[3] = throttle.Kp
    ...
    def pid(self):
        ...
        # Completed the equations for pitch and yaw axis
	...
	self.setpoint_euler[2] = self.setpoint_cmd[2] * 0.02 - 30
        ...
        # Computed error in each axis where error[0] corresponds to error in roll...
	...
	# Converting the throttle from the range of 1000 to 2000 to 0 - 1024
        ...
        # Calculate the pid output required for each axis. For eg: calcuate self.out_roll, self.out_pitch, etc.
        self.out_roll = self.Kp[0]*error[0] + (self.sample_time + error[0]) * self.Ki[0] + self.Kd[0]*(error[0] - self.prev_error[0])
        self.out_pitch = self.Kp[1]*error[1] + (self.sample_time + error[1]) * self.Ki[1] + self.Kd[1]*(error[1] - self.prev_error[1])
        self.out_yaw = self.Kp[2]*error[2] + (self.sample_time + error[2]) * self.Ki[2] + self.Kd[2]*(error[2] - self.prev_error[2])
	...
	# Motor mixing algorithm  
        ...
        # Use this computed output value in the equations to compute the pwm for each propeller. LOOK OUT FOR SIGN (+ or -). EXPERIMENT AND FIND THE CORRECT SIGN
        self.pwm_cmd.prop1 = self.setpoint_cmd[3] + self.setpoint_cmd[2] + self.setpoint_cmd[1] + self.setpoint_cmd[0]
        self.pwm_cmd.prop2 = self.setpoint_cmd[3] - self.setpoint_cmd[2] + self.setpoint_cmd[1] - self.setpoint_cmd[0]
        self.pwm_cmd.prop3 = self.setpoint_cmd[3] - self.setpoint_cmd[2] - self.setpoint_cmd[1] + self.setpoint_cmd[0]
        self.pwm_cmd.prop4 = self.setpoint_cmd[3] + self.setpoint_cmd[2] - self.setpoint_cmd[1] - self.setpoint_cmd[0]

        # Don't run the pid continously. Run the pid only at the a sample time. self.sampletime defined above is for this purpose. THIS IS VERY IMPORTANT.
        ...
        elif self.pwm_cmd.prop1 < self.min_values[0]: self.pwm_cmd.prop1 = self.min_values[0]
        if self.pwm_cmd.prop2 > self.max_values[1]: self.pwm_cmd.prop2 = self.max_values[1]
        elif self.pwm_cmd.prop2 < self.min_values[1]: self.pwm_cmd.prop2 = self.min_values[1]
        if self.pwm_cmd.prop3 > self.max_values[2]: self.pwm_cmd.prop3 = self.max_values[2]
        elif self.pwm_cmd.prop3 < self.min_values[2]: self.pwm_cmd.prop3 = self.min_values[2]
        if self.pwm_cmd.prop4 > self.max_values[3]: self.pwm_cmd.prop4 = self.max_values[3]
        elif self.pwm_cmd.prop4 < self.min_values[3]: self.pwm_cmd.prop4 = self.min_values[3]

        # Update previous errors. eg: self.prev_error[1] = error[1] where index 1 corresponds to that of pitch (eg)
        self.prev_error = [ error[0], error[1], error[2] ]

        # Add error_sum to use for integral component
        self.error_sum = sum(self.prev_error)
        # ------------------------------------------------------------------------------------------------------------------------
        
        # Publishing /roll_error, /yaw_error, /pitch_error
        self.roll_error_pub.publish(error[0])
        self.pitch_error_pub.publish(error[1])
        self.yaw_error_pub.publish(error[2])
        # self.throttle_error_pub.publish(error[3])

        # Publishing /edrone/pwm values
